...
import 'package:task_manager/API/Admin%20DataModel/companyLogDataModel.dart';
...
class CompanyLog extends StatefulWidget {
  final String id;
  const CompanyLog({required this.id, Key? key}) : super(key: key);

  @override
  State<CompanyLog> createState() => _CompanyLogState();
}
...
class TableSource extends AdvancedDataTableSource<CompanyLogDataModel> {
  ...
  DataRow? getRow(int index) {
    ...
    final CompanyLogDataModel dataList = lastDetails!.rows[index];
    ...
    return DataRow(
      cells: [
        ...
        DataCell(
          Container(
            width: deviceWidth * 0.5, // Adjust the width as needed
  ...
  @override
  Future<RemoteDataSourceDetails<CompanyLogDataModel>> getNextPage(
    ...
    if (dataModel != null && dataModel.status == true) {
      ...
      return RemoteDataSourceDetails(
        ...
        //count,
        dynamicData
            .map<CompanyLogDataModel>(
              (item) =>
                  CompanyLogDataModel.fromJson(item as Map<String, dynamic>),
            )
            .toList(),
        filteredRows: lastSearchTerm.isNotEmpty
            ? dynamicData
                .map<CompanyLogDataModel>(
                  (item) => CompanyLogDataModel.fromJson(
